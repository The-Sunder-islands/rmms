...
void AutomationClipView::disconnectObject( QAction * _a )
{
	...
	if( j && dynamic_cast<AutomatableModel *>( j ) )
	{
		...
		m_clip->objects().erase( std::find( m_clip->objects().begin(),
					m_clip->objects().end(),
...
void AutomationClipView::constructContextMenu( QMenu * _cm )
{
	...
	if (!m_clip->objects().empty())
	{
		_cm->addSeparator();
		auto m = new QMenu(tr("%1 Connections").arg(m_clip->objects().size()), _cm);
		for (const auto& object : m_clip->objects())
		{
			if (object)
			{
